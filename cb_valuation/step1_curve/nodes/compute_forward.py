...
dt 는 grid.tree.dt 하나만 쓴다(원천 단일). NODE_DISCOUNT_CONV 는 ③ 만 초안 지원(러너 사전 게이트)."""
import math
...
def node_compute_forward(s, C):
    if C.TREE_FWD_RULE != "continuous_from_spot" or C.NODE_DISCOUNT_CONV != "3_continuous_fwd":
        raise NotImplementedError(f"초안 미구현: TREE_FWD_RULE={C.TREE_FWD_RULE}, NODE_DISCOUNT_CONV={C.NODE_DISCOUNT_CONV}")
