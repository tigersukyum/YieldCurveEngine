...
화면 디자인은 `ref/design/DESIGN-dell-1996.md`(디자인 토큰: 검은 페이지 프레임, 평면 색블록 리본 카드, Arial Black 제목·Helvetica 굵은 UI 라벨·Times 본문, 모서리 0, 그림자 없음)를 따른다. 탭은 절차 순서(1 입력 → 2 승인 → 3 결과 → 4 증빙)이고 노란 "지금 여기" 스티커가 state 만 보고 볼 탭을 가리킨다. 빨간 블록은 페이지당 하나(실행)이며, 상단 배너 오른쪽 빨간 글씨가 현재 상태다.
